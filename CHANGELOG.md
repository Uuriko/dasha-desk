...
- **dasha bounties / the board** at `/bounties`: hunt a GitHub issue/PR or list a project with owner-written rules. Static feed at GitHub Pages `bounties/feed.json` (`schema: dasha-bounties-feed/v1`) and the same file as repo-root `bounties.json`. Also merges Demigod’s public listings JSON (GitHub raw / jsDelivr). Outcomes need a GitHub proof URL. No slop.cash scoring formula, no invented ranks. Declared, not escrow.
