...
- **This-week surfaces:** Home is ticker + stills (no Simp/quiz). Quiz/simp on `/lobby`. Native `/bounties` (List needs USDC + `payTo`). `/privacy`. `/desk` goes to `/dasha`. Branded `404.html`.
