...
- **dasha bounties / the board** at `/bounties/`: anybody lists a project and writes their own bounty rules (pool, what pays, eligibility, payout). Listings via seed file, GitHub issues, this-device localStorage, and shareable JSON. Optional public GitHub contributor counts — no slop.cash scoring formula, no invented ranks. Declared, not escrow.
