...
studio/         self-contained Meme Studio (not generated by build.mjs); loader.html is the Webflow first-paint paste
home/           pasteable Home: live ticker, one H1, one Studio CTA — no Simp board
lobby/          pasteable Lobby: Simp board + quiz
privacy/        pasteable /privacy
desk/           /desk → /dasha (host-aware redirect)
...
bounties.json           same feed at the Pages site root (/dasha-desk/bounties.json) — not www.getdasha.com/bounties.json
...
Listings come from `bounties/feed.json` (also published as GitHub Pages `/dasha-desk/bounties.json` and raw GitHub; **not** `www.getdasha.com/bounties.json`, which is Webflow’s page JSON export), open GitHub issues (`bounty-project` / `[bounty]`), this-device
