...
studio/         self-contained Meme Studio (not generated by build.mjs)
bounties/       self-contained ranked board (index.html + board.js + board.css)
config/bounties.seed.json   first listing so the board is not empty
```
...
Listings are open GitHub issues on `Uuriko/dasha-desk` (label `bounty-project` or title prefix
`[bounty]`) plus `config/bounties.seed.json`. Scores are computed in the browser from public GitHub
activity for the current UTC month. The page never connects a wallet, never constructs a
transaction, and never invents ranks or dollar amounts when GitHub fails. If a mint/CA appears here,
it must come from `config/dasha.json` — do not paste a second address into the markup.
...
| `bounties/bounties.test.mjs` | Listing JSON parses, malformed issues are skipped, failed GitHub fetches do not fabricate ranks or dollars, the form builds a GitHub `issues/new` URL, and the seed listing renders. |
...
```bash
python3 -m http.server 8766     # → http://127.0.0.1:8766/ and /bounties/ and /studio/
