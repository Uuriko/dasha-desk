...
bounties/feed.json      static listings feed the page also loads (`/bounties/feed.json`)
bounties.json           same feed at site root (`/bounties.json`)
...
localStorage, and `#l=` share JSON. Project-level pools are optional. Accepted outcomes need a GitHub PR, issue, or comment URL — no score without a link.
...
| `bounties/bounties.test.mjs` | Listing JSON parses (item + project), malformed issues are skipped, empty outcomes copy, proof URLs required, the form builds a GitHub `issues/new` URL, static feed matches root `/bounties.json`, seed has no fake leaderboard. |
