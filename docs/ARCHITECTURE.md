...
Anybody can list a project with owner-written rules (pool, what pays, eligibility, payout). Listings
come from `bounties/listings.json` / `config/bounties.seed.json`, open GitHub issues (`bounty-project` / `[bounty]`), this-device
localStorage, and `#l=` share JSON. Public GitHub contributor counts are optional and best-effort.
...
| `bounties/bounties.test.mjs` | Listing JSON parses, malformed issues are skipped, failed GitHub fetches do not fabricate ranks, the form builds a GitHub `issues/new` URL, seed listing renders without a fake leaderboard. |
