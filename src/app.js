(function () {
   ...
   * V55/V57: still-green → "Buy · Hard" / "Buy · Deep" / "Buy · still"; dump → "Dump".
   */
  function sizeChipHint(sol, dip) {
    ...
    // V57: buy-first chip tag when 24h still green (hard/deep/soft)
    ...
    return {
      ...
      hasStill: !!still,
      buyFirst: !!(still && still.short),
    };
  }

  /**
   * Sticky dip chip label — pure V57.
   * Raw "1h -5.97%" becomes "Buy · 1h -5.97%" during dip/dump.
   */
  function stickyDipLabel(dip) {
    if (!dip || dip.shortLabel == null || dip.shortPct == null) return null;
    var pct = Number(dip.shortPct);
    if (!isFinite(pct)) return null;
    var short =
      (pct > 0 ? '+' : '') +
      (Math.abs(pct) >= 10 ? pct.toFixed(0) : pct.toFixed(2)) +
      '%';
    return 'Buy · ' + dip.shortLabel + ' ' + short;
  }

  /**
   * Sticky 24h chip label — pure V57.
   * During dip with 24h still green: "24h +1.4% still"; else "24h +1.4%".
   */
  function stickyChLabel(ch24, dip) {
    var n = ch24 != null ? Number(ch24) : NaN;
    if (!isFinite(n)) return null;
    var core =
      '24h ' + (n > 0 ? '+' : '') + (Math.abs(n) >= 10 ? n.toFixed(0) : n.toFixed(2)) + '%';
    var still = dip && dip.kind !== 'dump' ? stillGreen24(dip, n) : null;
    if (still && still.short) return core + ' still';
    return core;
  }

  /**
   * Buy-first dump CTA lead — pure V51.
   * e.g. "Buy · Hard dump" for FOMO buy / sticky parity with dual (V50).
   */
  function dumpBuyLead(dip) {
    if (!dip || dip.kind !== 'dump') return null;
    var depth = dipDepth(dip);
    return depth && depth.tier !== 'soft'
      ? 'Buy · ' + depth.tag + ' dump'
      : 'Buy · Dump';
  }
  ...
  function fomoDipHeadline(dip, ch24Label, ab, bp, netShort) {
    ...
    // V53/V54: hard/deep always buy-first; soft buy-first when 24h still green
    if ((depth && depth.tier !== 'soft') || stillObj) {
      var lead =
        depth && depth.tier !== 'soft'
          ? dipBuyLead(dip) || 'Buy · ' + depth.tag + ' dip'
          : 'Buy · Soft dip';
      var bits = [lead];
      ...
      if (netShort && String(netShort).charAt(0) === '+') {
        ...
        bits.push(bp.pct + '% buys');
      } else if (!(depth && depth.tier !== 'soft')) {
        // Soft without net/buys: keep short TF drop as honest context
        bits.push(dip.shortLabel + ' ' + short);
      }
      bits.push('NFA');
      return bits.join(' · ');
    }
    // Soft, no still — A/B status vs buy-pressure
    if (ab === 'b') {
      var press =
    ...
    return dip.shortLabel + ' dip ' + short;
  }
  ...
  var DDShare = {
    ...
    stickyDipLabel: stickyDipLabel,
    stickyChLabel: stickyChLabel,
      ...
      .then(function (data) {
        ...
        if ($('dd-fomo-main') && $('dd-fomo')) {
          ...
          if (usedMove) {
            ...
            // V53/V54: buy-first class for hard/deep, or soft when 24h still green
            var stillNow = stillGreen24(lastProof.dip, lastProof.ch24n);
            var buyFirstMain =
              !!(depthNow && depthNow.tier !== 'soft') || !!stillNow;
            fomo.classList.toggle('is-buy-first', buyFirstMain);
            if ($('dd-fomo-ab')) {
              // Hide A/B when single buy-first format (hard/deep or soft+still)
              $('dd-fomo-ab').hidden = buyFirstMain;
              if (!buyFirstMain) {
          ...
          if ($('dd-sticky-dip')) {
            if (lastProof.dip) {
              ...
              // V57: buy-first sticky dip chip (honest short TF)
              $('dd-sticky-dip').textContent =
                stickyDipLabel(lastProof.dip) ||
        ...
        if ($('dd-sticky-ch')) {
          // V57: 24h still tag on sticky when dip+green
          var chLab =
            stickyChLabel(ch.h24, lastProof.dip) ||
            '24h ' + fmtPct(ch.h24);
          $('dd-sticky-ch').textContent = chLab;
  ...
  function wireBuyHrefs() {
        ...
        var bounceSticky =
          reclaimSticky && reclaimSticky.short
            ...
            : microSticky && microSticky.short
              ? microSticky.short
              : '';
        // V51: 5m bounce on sticky during dump (parity with FOMO main)
        if (
          isDumpSticky &&
          !bounceSticky &&
          isFinite(lastProof.m5n) &&
          lastProof.m5n >= 0.15
        ) {
          bounceSticky =
            '5m +' +
            (Math.abs(lastProof.m5n) >= 10
              ? lastProof.m5n.toFixed(0)
    ...
    if ($('dd-fomo-buy')) {
      ...
      if (showFomoBuy) {
        ...
        // V51: 5m bounce on FOMO buy during dump
        if (
          regime === 'dump' &&
          isFinite(lastProof.m5n) &&
          lastProof.m5n >= 0.15
