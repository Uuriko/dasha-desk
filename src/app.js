(function () {
  ...
  function paintPair(pair) {
    ...
    var mcap = num(pair.marketCap);
    if (mcap == null) mcap = num(pair.fdv);
  ...
  function refresh() {
    ...
    return fetch(DEX, { cache: 'no-store', signal: ctrl ? ctrl.signal : undefined })
      .then(function (r) {
  ...
  (function schedulePoll() {
    setTimeout(function () {
      if (typeof document === 'undefined' || document.visibilityState !== 'hidden') {
        // Wait for this result so the next delay includes its failure/reset.
