...
const ROUTES = [
  ...
  { method: 'POST', path: '/console/enroll', auth: 'session', unauth: 302,
    why: 'mints a single-use enrollment code for the signed-in account; the code is shown once' },
  ...
  { method: 'POST', path: '/admin/enroll', auth: 'admin_token', unauth: 401,
    why: 'operator path to mint an enrollment code for any account' },
  ...
  { method: 'POST', path: '/v1/provider/enroll', auth: 'none-by-design', unauth: null,
    why: 'exchanges a single-use enrollment code for a bound provider token; the code IS the credential. Unknown, used and expired are one answer' },
  { method: 'POST', path: '/console/recover', auth: 'none-by-design', unauth: null,
    why: 'asks for a link by email; same 302 whether or not the address has an account' },
  { method: 'GET', path: '/console/recover/confirm', auth: 'none-by-design', unauth: null,
    why: 'the emailed link; the token IS the credential. Read-only: it never consumes' },
  { method: 'POST', path: '/console/recover/confirm', auth: 'none-by-design', unauth: null,
    why: 'consumes the token once and mints a key; 400 for unknown, used and expired alike' },
...
function startGateway() {
  ...
  return createGateway({
    ...
    mailer: { send: async () => ({ messageId: 'stub' }) },
    secureCookies: false,
