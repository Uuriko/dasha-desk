...
assert.ok(!/<img\b|files\.catbox\.moe|cdn\.dexscreener\.com/.test(body), 'release body ships no unlicensed or provider-controlled media');
assert.ok(!/hardenImages|info\.imageUrl|dd-token-img/.test(src), 'runtime does not restore remote media');
...
assert.ok(src.includes("fetch(LABS, { method: 'HEAD', cache: 'no-store' })"), 'Labs link remains hidden until its route exists');
...
assert.equal(body.match(/id="dd-evidence-digest">([0-9a-f]{64})</)?.[1], evidence.account.accountDataSha256);
assert.ok(body.includes('Show how this was checked') && body.includes('node mint-evidence-refresh.mjs --check'));
assert.ok(body.includes('exactly 82 bytes') && body.includes('not token safety, ownership, endorsement, or timestamp inclusion'));
assert.ok(!/holder concentration|unique holders|whales/i.test(body), 'account-byte evidence makes no holder inference');
